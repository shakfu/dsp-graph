...
def _merge_params(
    ...
    for p in a.params:
        ...
        outer_params.append(Param(name=outer_name, min=p.min, max=p.max, default=p.default))
...
def parallel(a: Graph, b: Graph) -> Graph:
    ...
    for out in a.outputs:
        outer_outputs.append(AudioOutput(id=f"{a_id}_{out.id}", source=f"{a_id}__{out.id}"))
    for out in b.outputs:
        outer_outputs.append(AudioOutput(id=f"{b_id}_{out.id}", source=f"{b_id}__{out.id}"))
...
def merge(a: Graph, b: Graph) -> Graph:
    ...
    nodes: list[Subgraph | BinOp] = [sg_a]
