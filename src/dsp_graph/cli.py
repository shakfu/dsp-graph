...
def _cmd_simulate(args: argparse.Namespace) -> int:
    try:
        ...
        print(
            "error: numpy is required for simulation. Install with: pip install dsp-graph[sim]",
            file=sys.stderr,
        )
        return 1
    ...
    for spec in args.param or []:
        if "=" not in spec:
            print(f"error: invalid param spec (expected NAME=VALUE): {spec}", file=sys.stderr)
            return 1
    ...
    if args.input:
        ...
        for spec in args.input:
            if "=" in spec:
                ...
                if not unmapped:
                    print(f"error: no unmapped input for file: {wav_path}", file=sys.stderr)
                    return 1
    ...
    if input_ids and (not inputs or set(inputs.keys()) != input_ids):
        ...
        if missing:
            print(f"error: missing input(s): {', '.join(sorted(missing))}", file=sys.stderr)
            return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_sim.add_argument(
        "--param", action="append", metavar="NAME=VALUE", help="Set parameter (repeatable)"
    )
    p_sim.add_argument("--sample-rate", type=float, help="Override sample rate")
    p_sim.add_argument("--optimize", action="store_true", help="Optimize before simulation")
